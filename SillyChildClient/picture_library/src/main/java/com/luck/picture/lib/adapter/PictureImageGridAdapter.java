    ...
    private void changeCheckboxState(ViewHolder contentHolder, LocalMedia image) {
        ...
        if (selectImages.size() >= maxSelectNum && !isChecked) {
            ...
            @SuppressLint("StringFormatMatches") String str = eqImg ? context.getString(R.string.picture_message_max_num, maxSelectNum)
                    : context.getString(R.string.picture_message_video_max_num, 1);
            ToastManage.s(context, str);
            return;
        }

        if (selectImages.size() >= 1 && !isChecked && pictureType.startsWith(PictureConfig.VIDEO)) {
            @SuppressLint("StringFormatMatches") String str = context.getString(R.string.picture_message_video_max_num, 1);
            ToastManage.s(context, str);
            return;
        }
