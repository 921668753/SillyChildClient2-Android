...
import com.sillykid.app.utils.custompicker.holder.MonthTimeViewHolder;
...
public class MonthTimeAdapter extends RecyclerView.Adapter<MonthTimeViewHolder> {
    ...
    public void onBindViewHolder(MonthTimeViewHolder holder, int position) {
        ...
        holder.plan_time_txt_month.setText(monthTimeEntity.getYear() + context.getString(R.string.year) + monthTimeEntity.getMonth() + context.getString(R.string.month));  //显示 几年--几月
        //得到该月份的第一天
        ...
        for (int i = 1; i <= calendar.get(Calendar.DAY_OF_MONTH); i++) {     //添加 该月份的天数   一号 到 该月的最后一天
            if (position == 0) {
                if (i < Calendar.getInstance().get(Calendar.DAY_OF_MONTH)) {
                    days.add(new DayTimeEntity(i, monthTimeEntity.getMonth(), monthTimeEntity.getYear(), position, 100));//今天之前的都不能选择 用100来表示
                } else if (i == Calendar.getInstance().get(Calendar.DAY_OF_MONTH)) {
                    days.add(new DayTimeEntity(i, monthTimeEntity.getMonth(), monthTimeEntity.getYear(), position, 101));//今天的用101来表示
                } else {
