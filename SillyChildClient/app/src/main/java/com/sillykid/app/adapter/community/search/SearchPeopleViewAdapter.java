package com.sillykid.app.adapter.community.search;

import android.content.Context;

import com.sillykid.app.R;
import com.sillykid.app.entity.community.search.RecentSearchBean;

import cn.bingoogolapple.androidcommon.adapter.BGAAdapterViewAdapter;
import cn.bingoogolapple.androidcommon.adapter.BGAViewHolderHelper;

public class SearchPeopleViewAdapter extends BGAAdapterViewAdapter<RecentSearchBean.DataBean> {

    public SearchPeopleViewAdapter(Context context) {
        super(context, R.layout.item_recentsearchcommunity);
    }

    @Override
    protected void fillData(BGAViewHolderHelper helper, int position, RecentSearchBean.DataBean model) {

        helper.setText(R.id.tv_name, model.getName());

    }
}