...
public class AirportDropOffPresenter implements AirportDropOffContract.Presenter {
    ...
    @Override
    public void postAddRequirements(int product_id, String delivery_location, String flight_arrival_time, String contact, String contact_number,
        ...
        if (StringUtils.isEmpty(delivery_location)) {
            mView.errorMsg(KJActivityStack.create().topActivity().getString(R.string.fillPlaceDeparture1), 0);
            return;
        }
        if (StringUtils.toLong(flight_arrival_time) <= 0) {
            mView.errorMsg(KJActivityStack.create().topActivity().getString(R.string.selectDepartureTime), 0);
            return;
        }
        if (StringUtils.toLong(flight_arrival_time) < System.currentTimeMillis() / 1000) {
            mView.errorMsg(KJActivityStack.create().topActivity().getString(R.string.selectDepartureTime1), 0);
            return;
        }
